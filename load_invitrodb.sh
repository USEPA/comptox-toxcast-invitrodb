...
mysql_dir=$current_working_dir/invitrodb-loader/mysql/mysql-shell-8.0.17-linux-glibc2.12-x86-64bit
mysql_executable=$mysql_dir/bin/mysqlsh

echo Enter your MySQL login credentials
read -p "Hostname: " mysql_host
read -p "Username: " mysql_user
read -p "Port Number: " mysql_port

#rem set /p invitrodb_schema_name="What do you want to name the database schema? "
invitrodb_schema_name=invitrodb
invitrodb_ddl_file=$current_working_dir/prod_internal_invitrodb_v3_2.bare.sql
invitrodb_sql_file_dir=$current_working_dir/prod_internal_invitrodb_v3_2
...
$mysql_executable --sql $mysql_user@$mysql_host:$mysql_port/mysql --password -e "drop database if exists $invitrodb_schema_name; create database $invitrodb_schema_name;"
